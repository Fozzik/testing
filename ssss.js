      ...
      if (element.json) {
        ...
        Api.qualitys(element.video, function (data) 
		{
          if (data.error || data.kakogohyia) {
            Lampa.Noty.show(data.kakogohyia === 'IsNotValidUser' ? 'аккаунт заблокирован до конца текущего часа/дня' : Lampa.Lang.translate('torrent_parser_nofiles'));
            Lampa.Loading.stop();
            return;
          }	
          ...
          var video = {
            title: element.name,
            url: qualityDefault(qualitys) || element.video,
            url_reserve: data.qualitys_proxy ? qualityDefault(data.qualitys_proxy) || element.video_reserve || element.video : false,
            quality: qualitys
          };
